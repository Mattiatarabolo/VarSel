...
Job <- "1"#inputs[1]
which_prior <- "unif"#inputs[2]  #(options "unif" "norm"  "exp")
...
phylo <- readRDS(paste("phylo_40_", extinction_rate, "_Temperature_",  Job, ".rds", sep = ""))
...
env_list <- list("Temperature" = InfTemp)
env_num_tot <- length(env_list)

env_names <- c("Temperature")
...
par_names <- c("lambda_0", "mu_0", "G_T")

pamhLocalName = paste("tracer_logfile_", extinction_rate, "_", which_prior, "_", Job, sep = "")#paste("/data/biodiv/tarabolo/bayes_bench/outdata/tracer_logfile/tracer_logfile_", extinction_rate, "_", which_prior, "_", Job, sep = "")
...
sampler <- run_env_bd_MCMC(tree = phylo, f = f_total, f.lamb = f.lamb, f.mu = f.mu, prior = priorDensity, start_gen = parGen, 
                           ...
                           thin = 1e2, update = 1e2, adaptation = 1e4, max_iter = 1e6, seed = NULL, nCPU = 1)
